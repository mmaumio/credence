/* global credBgColors, credThemeColor, jQuery, wp, _ */
/**
...
( function() {
	...
	wp.customize.bind( 'ready', function() {
		...
		wp.customize( 'accent_hue', function( value ) {
			value.bind( function( to ) {
				...
				Object.keys( credBgColors ).forEach( function( context ) {
					var backgroundColorValue;
					if ( credBgColors[ context ].color ) {
						backgroundColorValue = credBgColors[ context ].color;
					} else {
						backgroundColorValue = wp.customize( credBgColors[ context ].setting ).get();
					}
					credThemeSetAccessibleColorsValue( context, backgroundColorValue, to );
				} );
			} );
		} );
		...
		Object.keys( credBgColors ).forEach( function( context ) {
			wp.customize( credBgColors[ context ].setting, function( value ) {
				value.bind( function( to ) {
					// Update the value for our accessible colors for this area.
					credThemeSetAccessibleColorsValue( context, to, wp.customize( 'accent_hue' ).get(), to );
				} );
			} );
		} );
	} );
	 ...
	 * @return {void}
	 */
	function credThemeSetAccessibleColorsValue( context, backgroundColor, accentHue ) {
		var value, colors;
		...
		colors = credThemeColor( backgroundColor, accentHue );
